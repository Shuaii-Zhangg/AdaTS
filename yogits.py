...
class YOGITS(Optimizer):
    r"""Implements YOGITS algorithm.
    Arguments:
    ...
    """
    def __init__(self, params,  lr=1e-2, betas=(0.9, 0.999), eps=1e-3, final_lr=0.01,
        ...
        super(YOGITS, self).__init__(params, defaults)
    ...
    def step(self, closure=None):
        ...
        for group, base_lr in zip(self.param_groups, self.base_lrs):
            for p in group['params']:
                ...
                exp_avg_var.addcmul_((beta2 - 1), torch.sign(exp_avg_var - grad2), grad2)
                torch.max(max_exp_avg_var, exp_avg_var / bias_correction2, out=max_exp_avg_var)
                denom = group['lr'] / (max_exp_avg_var.sqrt()+group['eps'])
